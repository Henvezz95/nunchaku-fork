...
#pragma once
#include "ops.h" 
#include "kernels/zgemm/gemm_w4a4.cuh"
...
namespace nunchaku::ops {
    ...
    std::tuple<torch::Tensor, torch::Tensor> quantize_w4a4_wgt(torch::Tensor input) {
        ...
        kernels::quantize_w4a4_wgt_wrapper(nunchaku_input, qweight, wscales);

        return std::make_tuple(to_torch(qweight), to_torch(wscales));
    }

    std::tuple<torch::Tensor, torch::Tensor> quantize_w4a4_act(torch::Tensor input) {
        Tensor nunchaku_input = from_torch(input.contiguous());
        const int M = nunchaku_input.numel() / nunchaku_input.shape[-1];
        const int K = nunchaku_input.shape[-1];
        Tensor qact = Tensor::allocate({M, K / 2}, Tensor::INT8, nunchaku_input.device());
        Tensor ascales = Tensor::allocate({M * K / 64}, nunchaku_input.dtype(), nunchaku_input.device());
        
        // Call the new, simple wrapper function
        kernels::quantize_w4a4_act_wrapper(nunchaku_input, qact, ascales);

        return std::make_tuple(to_torch(qact), to_torch(ascales));
    }
...
torch::Tensor gemm_w4a4_lr_fp16(
    ...
    auto out_shape = {act.size(0), wgt.size(0)};
    torch::Tensor out = torch::empty(out_shape, act.options());

    // 2. Call the generic gemm_w4a4 function, passing your arguments
    //    and std::nullopt for all unused parameters.
    gemm_w4a4(
        act, wgt, out, std::nullopt, ascales, wscales, std::nullopt,
        std::nullopt, std::nullopt, lora_up, lora_down, std::nullopt,
        std::nullopt, std::nullopt, std::nullopt, bias, std::nullopt,
        std::nullopt, std::nullopt, false, {}, false, false, 1.0,
